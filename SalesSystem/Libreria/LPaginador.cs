...
namespace SalesSystem.Libreria
{
    public class LPaginador<T>
    {
        ...
        //definimos que ira en el enlace de la pagina anterior
            ...
            String action, String host)
        {
            ...
            if (pagi_actual != 1)
            {
                ...
                int pagi_url = 1; //sera el numero de pagina al que enlazemos
                pagi_navegacion += "<a class='btn btn-default' href='" + host + "/" + controller + "/" + action + "?id=" + pagi_url +
                    "&registros=" + pagi_cuantos + "&area=" + area + "'>" 
                    + pagi_nav_primera + "</a>";

                //Si no estamos en la pagina 1. Ponemos el enlace "anterior"
                pagi_url = pagi_actual - 1; //sera el numero de pagina al que enlazemos
            ...
            for (int pagi_i = pagi_nav_desde; pagi_i <= pagi_nav_hasta; pagi_i++)
            {
                ...
                {
                    //Si el numero de pagina es la actual (pagi_actual). Se escribe el numero, pero sin enlace y en negrita
